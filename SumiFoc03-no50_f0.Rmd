...
SumiFoc.no50.f03 <- lm(T50 ~ Focus * ToneCondition, data = SumiFoc.no50.wide)
...
SumiFoc.no50.f0.plot3 <- ggplot(data = SumiFoc.no50.wide, aes(x = Focus, fill = ToneCondition,  y = T50)) +
  geom_boxplot() +
  theme_bw() +
  labs(x = "Focus condition", y = "F0 (Hz)", title = (expression(paste("F0 at midpoint of ", italic("=no"), ", by focus condition and tone category of preceding syllable")))) +
  scale_fill_discrete(name="Tone of\npreceding syll",
                         breaks=c("H", "M", "L"),
                         labels=c("High", "Mid", "Low")) +
  theme(plot.title = element_text(hjust = 0.5))
SumiFoc.no50.f0.plot3
dev.copy(png, "figures/SumiFoc_no50_focustone_f0.png")
dev.off()
```
